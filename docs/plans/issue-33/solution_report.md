...
## 1. 🚨 Context & Problem
The legacy flat codebase layout mixed HTTP webhooks, Obsidian API calls, Gemini NLP prompts, and state logic in monolithic service files (`src/services/`, `src/obsidian/`, `src/gemini/`), making testing difficult and causing regression bugs during system expansion.
...
2. **Governance Layer (`src/governance/`)**: Probabilistic intent routing ($\ge 95\%$) and D1/KV HITL session state (< 95%).
3. **Tool Layer (`src/tools/`)**: Deterministic client wrappers (Obsidian, D1/KV, Google Calendar, Telegram).
...
- `src/tools/` (`obsidianClient.ts`, `d1Client.ts`, `googleClient.ts`, `telegramClient.ts`)
