...
Người dùng yêu cầu khi thêm task, thuộc tính `Date` không chỉ lưu ngày (VD: `2026-06-23`) mà lưu kèm theo cả thời gian (VD: `2026-06-23T15:00:00+08:00`). Việc này giúp Obsidian hiển thị chính xác vị trí của task trên giao diện Calendar view.
...
### 3. Cập nhật `src/obsidian/client.ts`
- Thuộc tính `Date` của Obsidian API vốn đã hỗ trợ chuỗi ISO 8601 chứa Time. Nên `createTask` không cần thay đổi logic mapping (`Date: { date: { start: task.dueDate } }`). Nó sẽ tự động nhận diện thời gian. Tuy nhiên có thể kiểm tra xem có cần update type hay parse cho cẩn thận không.
...
3. **Cập nhật tài liệu:** Sẽ cập nhật file `obsidian_database_setup.md` để ghi chú về thuộc tính `Date` bao gồm cả Time.

## Verification Plan
- Chạy thử lệnh `/add_task Đọc sách lúc 20:00` -> Mở Obsidian kiểm tra xem thuộc tính Date có kèm thời gian `8:00 PM` không.
