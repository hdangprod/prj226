# Solution Report: Issue #52 — OpenWiki Private Vault Setup & Native Obsidian-to-OKF Synthesis (Final)
...
3. **Obsidian Permission Lock**: When the Obsidian database/page was not shared with the Obsidian Integration (`Gemini Assistant`), the Obsidian API returned empty search results, sending Telegram alerts stating `✅ 0 wiki entries synthesized`.
4. **Orphaned File Accumulation**: Deleting a page in Obsidian left behind old `.md` files in GitHub `./vault/`, leading to ghost data in downstream vector searches.
...
1. **Native Node.js Synthesizer Engine ([`scripts/sync-obsidian-to-vault.js`](file:///Users/dangnguyen/Desktop/PRJ226/scripts/sync-obsidian-to-vault.js))**:
   - Queries Obsidian REST API directly using `GITHUB_TOKEN`.
   - Filters pages by targeted Obsidian Database (`VAULT_DIR="3832a737c87d80a08177c6285594dbcf"`).
   - Extracts page titles, metadata, and block text, generating Open Knowledge Format (OKF v0.1) Markdown files in `./vault/`.
   - Implements **Vault Reconciliation**: automatically identifies and deletes orphaned `.md` files from `./vault/` when a page is deleted from Obsidian.
...
3. **Workflow Permission & Secret Configuration**:
   ...
   - Injected secrets: `GITHUB_TOKEN`, `GOOGLE_API_KEY`, `TELEGRAM_BOT_TOKEN`, `TELEGRAM_CHAT_ID`.
...
- `GITHUB_TOKEN`: Obsidian Integration secret (`ntn_...`).
...
#### Step 5: Add Obsidian Database Connection
In Obsidian, open the **Resources** database → Click `...` (top right) → **Connections** → Add **Gemini Assistant**.

---

### 4. Blast Radius
- Added [`scripts/sync-obsidian-to-vault.js`](file:///Users/dangnguyen/Desktop/PRJ226/scripts/sync-obsidian-to-vault.js).
