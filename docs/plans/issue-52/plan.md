...
This document outlines the architecture, setup instructions, issue resolution guide, and implementation details for orchestrating a private GitHub knowledge vault (`hdangprod/hdangprod_wiki`) using a native Node.js Notion-to-OKF synthesis engine.
...
> - Notion REST API integration replaces PyPI stub package `openwiki` for deterministic, zero-dependency node synthesis.
...
1. `NOTION_TOKEN`: Notion Integration Secret API Key (`ntn_...`).
...
| :--- | :--- | :--- | :--- |
| **Trap 1: PyPI Stub Package** | Log trace shows `openwiki personal` outputting `"openwiki"` and exiting without creating `./vault/`. | PyPI package `openwiki==0.1.1` is a 5-line print placeholder module. | Replace `openwiki` CLI with native Node script `scripts/sync-notion-to-vault.js`. |
...
| **Trap 3: Notion Database Permission Lock** | Telegram alert reports `✅ 0 wiki entries synthesized`. | Notion Integration not added under **Connections** on the database page. | Click `...` → **Connections** → Add your Notion Integration (e.g. `Gemini Assistant`). |
| **Trap 4: Orphaned Files on Deletion** | Notes deleted in Notion still remain in `./vault/` on GitHub. | One-way sync without vault directory state reconciliation. | Add state reconciliation in `sync-notion-to-vault.js` to delete `.md` files missing from Notion. |
...
### [Component: Scripts]
#### [NEW] [sync-notion-to-vault.js](file:///Users/dangnguyen/Desktop/PRJ226/scripts/sync-notion-to-vault.js)
Native Node.js script querying Notion REST API, fetching block text, generating OKF Markdown entries, and reconciling deleted vault files.
...
Updated workflow YAML adding `permissions: contents: write` and executing `node scripts/sync-notion-to-vault.js`.

### [Component: Documentation & Sitemap]
#### [MODIFY] [sitemap.md](file:///Users/dangnguyen/Desktop/PRJ226/docs/sitemap.md)
Updated directory structure and script indexes to include `scripts/sync-notion-to-vault.js`.
