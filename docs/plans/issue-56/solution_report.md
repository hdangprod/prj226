...
## 1. Context & Problem
PRJ226 v3.0 relied on Obsidian API and Neon Postgres. This setup resulted in cold start delays, 200–500ms retrieval latencies in the hot path, Obsidian rate limits, and Git commit noise on per-message captures.
...
- **Deleted (6 files)**: `obsidianClient.ts`, `neonClient.ts`, `debounceBuffer.ts`, `sync-obsidian-to-vault.js`, `index-vault-to-neon.js`, `types/obsidian.ts`.
